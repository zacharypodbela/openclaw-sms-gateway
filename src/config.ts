...
export function parseConfig(pluginConfig: Record<string, unknown> | undefined): SmsGatewayConfig {
  ...
  const apiUrl = optionalString(raw, "apiUrl") ?? "https://api.sms-gate.app/3rdparty/v1";
  const defaultSimNumber = optionalInt(raw, "defaultSimNumber") ?? 1;
  const webhookPath = optionalString(raw, "webhookPath") ?? "/plugins/sms-gateway/webhook";
...
function expectString(raw: Record<string, unknown>, key: string): string {
  ...
  if (typeof value !== "string" || !value.trim()) {
    throw new Error(`sms-gateway: missing required config field "${key}"`);
  }
  return value.trim();
}
...
function optionalString(raw: Record<string, unknown>, key: string): string | undefined {
  ...
  if (typeof value !== "string") {
    throw new Error(`sms-gateway: config field "${key}" must be a string`);
  }
...
function optionalInt(raw: Record<string, unknown>, key: string): number | undefined {
  ...
  if (typeof value !== "number" || !Number.isInteger(value)) {
    throw new Error(`sms-gateway: config field "${key}" must be an integer`);
  }
  return value;
}
